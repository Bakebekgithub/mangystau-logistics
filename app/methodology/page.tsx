    ...
    <>
      ...
      <main className="mx-auto max-w-3xl space-y-8 px-4 py-8">
        ...
        <Section title="Допущения расчёта">
          ...
          <Row
            label="Дозагрузка попутного рейса"
            value="крюк ≤ 120 км, остаток ≥ половины платы"
            source="Правила продукта, заявлены открыто"
          >
            Когда у перевозчика уже есть своя договорённость, коридор задан и менять его
            нельзя. Кандидаты оцениваются по крюку — насколько заезд за грузом удлиняет
            маршрут, — а не по полному расстоянию, и ранжируются по остатку после топлива
            на этот крюк, а не по объявленной сумме: жирная цена в стороне от трассы стоит
            меньше скромной на ней. Груз, у которого крюк съедает больше половины платы,
            не показывается: в плюсе, но ради этого никто не поедет.
          </Row>
          <Row
            label="Какой груз в какой кузов"
            value="4 класса груза"
            source="Заявленное правило, огрублённое сознательно"
          >
            Веса и рефрижератора недостаточно: самосвалу предлагалась питьевая вода,
            рефрижератору — кирпич. Груз классифицируется по описанию на скоропорт,
            навалочный, длинномерный и упакованный, и каждый кузов берёт только свои
            классы. Самосвал — только навал; открытая бортовая — не еду; рефрижератор —
            не песок и не арматуру. Правило намеренно грубое: четырёх классов хватает,
            чтобы убрать абсурд, а каждый следующий пришлось бы защищать, не зная
            реального парка области.
          </Row>
          <Row
            label="Кто ставит цену"
            value="заказчик"
            source="Решение продукта, а не расчёт"
          >
            Мы не знаем фрахтовых ставок Мангистау и не выдумываем их. Цену в заявке пишет
            заказчик, перевозчик соглашается или предлагает свою — как это и происходит
            сегодня по телефону. Платформа считает только пол: ниже него перевозчик едет
            в убыток.
          </Row>
          <Row
            label="Рекомендованный минимум"
            value={`топливо ÷ ${ASSUMPTIONS.fuelShareOfOperatingCost} × доля кузова`}
            source="Считается из реальных расстояний, доля топлива — допущение"
          >
            {ASSUMPTIONS.minChargedShareNote}
          </Row>
          <Row
            label="Выручка рейса"
            value="сумма цен заказчиков"
            source="Из данных, без тарифа"
          >
            То, что водитель видит в карточке, — это сумма того, что предложили заказчики
            в этом рейсе, с разбивкой по каждому плечу. Ни одна цифра заработка не выдумана
            движком.
          </Row>
        </Section>
